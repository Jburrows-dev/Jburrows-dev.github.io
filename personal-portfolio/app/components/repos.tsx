...
export default function Repos({ repos }: { repos: Array<repo> }) {
    return (
        <section id="project_sec" className="blog-sec sec-pad-top-sm">
          ...
          <div className="mdl-card mdl-shadow--2dp !p-0 repo-card">
            <div className="mdl-card__title !p-0">
              <Image className="blog-img" alt={r.name} loading="lazy" src={r.banner} width={250} height={150}/>
            </div>
            <div className="mdl-card__supporting-text relative">
              <span className="blog-cat" style={{ display: r.lang ? 'inline' : 'none' }}>
                <span className="lang" style={{backgroundColor: r.color}}></span>
                <span>{r.lang}</span>
              </span>
              <a href={r.url}>
                <h4 className="mt-15 mb-20">{r.name}</h4>
              </a>
              <p>{r.description}</p>
              <a href={r.url} className="mdl-button mdl-js-button mdl-button--fab mdl-js-ripple-effect bg-gray mdl-shadow--8dp" data-upgraded="MaterialButton,MaterialRipple">
